...
const localWorkerCRUD = (_endpointConfig: SparqlEndpoint, options: { doQuery: DoQuery} ): CRUDFunctions => {
...
const workerProvider: Record<SparqlEndpoint['provider'], <T = Record<string, any>>(endpointConfig: SparqlEndpoint, options?: T ) => CRUDFunctions > = {
...
const getProviderOrDefault = (endpoint: SparqlEndpoint) => typeof workerProvider[endpoint.provider] === 'function' ? workerProvider[endpoint.provider] : oxigraphCrudOptions
...
export const useGlobalCRUDOptions: UseGlobalCRUDOptions = () => {
  ...
  useEffect(() => {
    setCrudOptions(
      activeEndpoint && (
        getProviderOrDefault(activeEndpoint)(activeEndpoint,
