...
const LayoutElement = ({
  ...
  if (child.type === "Control") {
    ...
    i18nDescription = getI18nDescription(
      null,
      translator,
      child,
      childPath,
  ...
  const visible: boolean = hasShowRule(child)
    ? isVisible(child, rootData, path, getAjv(state))
    : true;
  return (
    <Grid item key={`${path}-${index}`} xs>
      <Grid container direction={"column"}>
        {i18nDescription && i18nDescription.length > 0 && visible && (
          <Grid item xs>
            <FormHelperText>
              <MDEditorMarkdown
...
export const LayoutWithDescriptionRenderer = React.memo(
