...
const Person = ({ person }: { person: any }) => {
  return (
    <dd className="persons">
      <Link href="#">
        {person.name || ""} {format(person.birth_date)}{" "}
        {person.death_date ? `- ${format(person.death_date)}` : ""}
      </Link>
    </dd>
  );
};
        ...
        <div className="container">
          <nav className="breadcrumb">
            <ol>
              ...
              <li>
                <Link href="/exhibition" title="Veranstaltung">
                  Ausstellung
                </Link>
              </li>
            </ol>
              ...
              <dl>
                ...
                <dd className="title">
                  ...
                  <span className="additional-block">
                    {format(exhibition?.start_date?.date)} -{" "}
                    {format(exhibition?.end_date?.date)}{" "}
                  </span>
                </dd>
                ...
                <dt
                  id="event_44711_corporations"
                  className="label-corporations"
                >
                  Beteiligte Körperschaften
                </dt>
                <dd className="corporations"></dd>
                <dt id="event_44711_persons" className="label-persons">
                  Kurator
                </dt>
                {!Array.isArray(exhibition.curator)
                  ? null
                  : exhibition.curator.map((person: any) => (
                ...
                {!Array.isArray(exhibition.artist)
                  ? null
                  : exhibition.artist.map((person: any) => (
                      <Person key={person["@id"] || ""} person={person} />
                    ))}
