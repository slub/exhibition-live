...
export const SearchBar = () => {
  ...
  const { data: allExhibitions } = useQuery(
    ["allExhibitions", searchText],
    async () => {
      if (!selectFetch) return [];
      const searchString = searchText.toLowerCase().replace(/"/g, "");
      const query = withDefaultPrefix(
        defaultPrefix,
        SELECT.DISTINCT`?s ?title ?to ?from `.WHERE`
  ...
  const dateScore = useMemo(() => {
    const allYears = uniq(
      filterUndefOrNull(
        allExhibitions?.flatMap((item) => [item.to?.value, item.from?.value]) ||
          [],
      ).map(dateValueToInt),
    );
    return orderBy(
      allYears.map((year) => {
        const exhibitions = allExhibitions?.filter(
          (item) =>
            dateValueToInt(item.to?.value || "0000") === year ||
            dateValueToInt(item.from?.value || "0000") === year,
        );
        ...
        <Grid container spacing={2}>
          {
            <Paper
              sx={{
                ...
                backgroundColor: (theme) =>
                  `rgba(255, 255, 255, ${dateScore?.length <= 0 ? 0 : 0.8})`,
                display: "flex",
                borderRadius: "1rem",
                alignItems: "center",
                width: "100%",
            ...
            >
              <ParentSize>
                {({ width, height }) =>
                  dateScore?.length <= 0 ? (
                    <Skeleton sx={{ width, height: "300px" }} />
                  ) : (
