...
import {useFormData, useFormEditor, useGlobalSearch, useOxigraph} from '../../state'
...
const TypedForm = ({defaultData, typeName, classIRI}: MainFormProps) => {
  ...
  const mainFormRenderers = useMemo(() => {
    return [
      materialCategorizationStepperLayoutWithPortal(stepperRef?.current, actionRef?.current)]
  }, [stepperRef?.current, actionRef?.current])
