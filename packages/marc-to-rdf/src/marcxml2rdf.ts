...
const findMappingForSubfield: (tag: string) => any | null = (tag: string) => {
  const tagMappingKey: string | undefined = Object.keys(
    mappingSkeleton.tags,
  ).find((tagPattern) => new RegExp(tagPattern).test(tag));
  return tagMappingKey ? mappingSkeleton.tags[tagMappingKey] : null;
};

export const kxp: Record<string, NamedNode<string>> & {
  (property?: TemplateStringsArray | string): NamedNode;
} = namespace("http://kxp.k10plus.de/ontology/");
...
export const mapDatafieldToQuads: (
  subjectNode: NamedNode<string>,
  ds: Datafield,
) => Quad[] = (subjectNode, ds) => {
  const tag = ds["@_tag"];
  ...
  if (mapping) {
    if (mapping.subfield) {
      Object.entries(mapping.subfield).forEach(
        ([code, subfieldMapping]: [string, any]) => {
          if (isArray(ds.subfield)) {
            const text = ds.subfield
            ...
            if (subfieldMapping.relation) {
              ...
              if (conditions) {
                const possiblePredicates = filterUndefOrNull(
                  Object.entries(conditions.subfield).map(
                    ([subfieldCode, condSubfieldMapping]: [string, any]) => {
                      const { default: default_, subs } = condSubfieldMapping;
                      const subKeys = Object.keys(subs || {});
                      if (isArray(ds.subfield)) {
                        const subfieldText = ds.subfield
                          .filter((sf) => sf["@_code"] === subfieldCode)
                          .map((sf) => sf["#text"]);
              ...
              quads.push(
                df.quad(subjectNode, makeIRI(realPredicate), objectNode),
              );
              quads.push(df.quad(objectNode, rdf.type, makeIRI(clazz)));
              if (subfield) {
                Object.entries(subfield).forEach(
                  ([key, value]: [string, any]) => {
                    const subText = (ds.subfield as SubfieldElement[])
                      .filter((sf) => sf["@_code"] === key)
                      .map((sf) => sf["#text"]);
                    subText.forEach((t) => {
                      quads.push(
                        df.quad(
                          objectNode,
                          makeIRI(value.predicate),
                          toLiteral(t),
                        ),
                      );
                    });
            ...
            text.forEach((t) => {
              quads.push(
                df.quad(
                  subjectNode,
                  makeIRI(subfieldMapping.predicate),
                  toLiteral(t),
                ),
              );
            });
