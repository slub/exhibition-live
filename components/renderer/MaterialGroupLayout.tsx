...
import {Card, CardContent, CardHeader, FormHelperText, Grid, Hidden} from '@mui/material'
...
const GroupComponent = React.memo(({ visible, enabled, uischema, state, path, schema, ...props }: MaterialLayoutRendererProps & {state: JsonFormsState}) => {
  ...
  const i18nLabel = path && getI18nLabel(
  ...
  const i18nDescription = path && getI18nDescription(null, translator, groupLayout, `${path}.${groupLayout.label}`, resolvedSchema)
  const rehypePlugins = useMemo<PluggableList>(() => [[rehypeSanitize],[rehypeExternalLinks, { target: '_blank' }]], [])
  return (
    <Hidden xsUp={!visible}>
      <Card style={style}>
        {!isEmpty(i18nLabel) && (
          <CardHeader title={i18nLabel} />
        )}
        <CardContent>
          ...
          <MaterialLayoutRenderer {...props} path={path} schema={schema} visible={visible} enabled={enabled} elements={groupLayout.elements} />
        </CardContent>
      </Card>
    </Hidden>
  )
