...
import {DeclarativeMappings, StrategyContext, strategyFunctionMap} from './mappingStrategies'

export const mapByConfig = async (sourceData: Record<string, any>, targetData: any, mappingConfig: DeclarativeMappings, strategyContext: StrategyContext): Promise<any> => {
    ...
    if (sourceValue) {
      if (expectedSchema && !ajv.validate(expectedSchema, sourceValue)) {
        ...
        if (!mapping?.strategy) {
          ...
          const value = await mappingFunction(sourceValue, get(newData, targetPath), strategyOptions, strategyContext)
