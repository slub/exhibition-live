...
export const buildWikidataFulltextSearchRestParams: (searchString: string, haswbstatement: string[][], limit?: number) => URLSearchParams
    = (searchString, haswbstatement, limit) =>
    new URLSearchParams({
...
type WikidataRESTResult = {
  pages: {
    thumbnail:
        { duration: number | null; size: null; width: number; mimetype: string; url: string; height: number };
...
export const findPersonWithinWikidataUsingREST = async (searchString: string, limit?: number, typeOf: string = 'Q5') => {
  const result = await fetch(`${wikidataRestAPIURL}?${buildWikidataFulltextSearchRestParams(searchString, [[`P31=${typeOf}`]], limit)}`)
      .then(res => res.json())
