...
import {ArrayDataProp, withJsonFormsArrayLayoutProps} from './withJsonFormsArrayLayoutProps'
...
const SelectListWithChipsRenderer = ({
  ...
  useEffect(() => {
    ...
    setEnumLabels(Object.fromEntries(schema.enum.map(key =>
        [key, i18nHelper(buildItemsEnumKey(key), key, translator, uischema, path, schema)])))
    setOptions(schema.enum.map((key) => ({
      key: key as string,
      value: key,
      label: i18nHelper(buildItemsEnumKey(key), key, translator, uischema, path, schema) || key
    })))
