...
export const mapStateToMasterListItemProps = (
  ...
  const findFirstPrimitiveProp = () =>
    schema.properties
      ...
      : undefined;
  const labelProp: any =
    uischema.options?.elementLabelProp || findFirstPrimitiveProp();
  ...
  if (Array.isArray(labelProp)) {
    childLabel = filterUndefOrNull(
      labelProp.map(
        (prop) =>
          typeof prop === "string" && resolveObj(childData, prop, undefined),
      ),
    ).join(labelJoint);
  } else {
